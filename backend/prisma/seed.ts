...
'''`.trim()
  ...
  await prisma.prompt.create({
    data: {
      ...
      template: `Seu papel é gerar uma descrição sucinta para um vídeo que será postado no LinkedIn.
  
Abaixo você receberá uma transcrição desse vídeo, use essa transcrição para gerar a descrição.

A descrição deve possuir no máximo 200 palavras em primeira pessoa contendo os pontos principais do vídeo.
